...
import { ServerRequests } from "../httpRequests/allTypeOfServerRequests";
...
export class JobsPage implements OnInit {
  ...
  constructor(
    ...
    public serverReq: ServerRequests
  ) {}
  ...
  JobPosts(id) {
    this.http
      .get(this.url.fetching_job_posts + id, { headers: this.headers })
      .toPromise()
      .then(
        res => {
          var posts = res.json();
          if (posts.message == "OK") {
            ...
            if (this.jobposts.length > 1) {
              this.jobposts = this.jobposts.sort(
                (a, b) =>
            ...
            this.Error_Msg =
              "Update your profile to get posts related to you or search!!!";
            this.connection = false;
          }
        },
        err => {
    ...
    if (this.search_value == "") {
      ...
      this.http
        ...
        .then(
          ...
          err => {
            this.Error_Msg =
              "Something went wrong, refresh the page or try diff search!!!";
            this.connection = false;
          }
        );
    }
  ...
  OpenDetails(index) {
    ...
    this.navCtrl
      .push(JobpostDetailsPage, { moredata: data })
      .then(() => {
